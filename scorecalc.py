...
def one_yr_liabilities_growth_score(balance_sheet : dict):
    """Returns the score attributed to a company's 1-year liabilities growth"""
    val = one_yr_liabilities_growth(balance_sheet)
    return (standardized_normal_dist(val, 20, -57)) / (0.420851 / val)


def two_yr_liabilities_growth_score(balance_sheet : dict):
    """Returns the score attributed to a company's 2-year liabilities growth"""
    val = two_yr_liabilities_growth(balance_sheet)
    return (standardized_normal_dist(val, 20, -14)) / (5.25505 / val)
